...
class TestPeeps < Minitest::Test
	...
	def test_changelog_nodupe
		DB.exec("INSERT INTO peeps.changelog(person_id, schema_name, table_name, table_id) VALUES (1, 'now', 'urls', 1)")
		res = DB.exec("SELECT * FROM peeps.changelog WHERE person_id=1")
		assert_equal 1, res.ntuples
		assert_equal '1', res[0]['id']
		DB.exec("INSERT INTO peeps.changelog(person_id, schema_name, table_name, table_id) VALUES (1, 'now', 'urls', 1)")
		res = DB.exec("SELECT * FROM peeps.changelog WHERE person_id=1")
		assert_equal 1, res.ntuples
		DB.exec("UPDATE peeps.changelog SET approved=TRUE WHERE id=1")
		DB.exec("INSERT INTO peeps.changelog(person_id, schema_name, table_name, table_id) VALUES (1, 'now', 'urls', 1)")
		res = DB.exec("SELECT * FROM peeps.changelog WHERE person_id=1")
		assert_equal 2, res.ntuples
	end

	def test_outgoing_email
		# PARAMS: emailer_id, person_id, profile, category, subject, body, reference_id
		err = assert_raises PG::RaiseException do
			DB.exec("SELECT * FROM peeps.outgoing_email(1, 999, 'sivers', 'sivers', 'x', 'x', NULL)")
		end
		assert err.message.include? 'person_id not found'
		err = assert_raises PG::RaiseException do
			DB.exec("SELECT * FROM peeps.outgoing_email(1, 1, 'sivers', ' ', 'x', 'x', NULL)")
		end
		assert err.message.include? 'category must not be empty'
		err = assert_raises PG::RaiseException do
			DB.exec("SELECT * FROM peeps.outgoing_email(1, 1, 'sivers', 'sivers', ' ', 'x', NULL)")
		end
		assert err.message.include? 'subject must not be empty'
		err = assert_raises PG::RaiseException do
			DB.exec("SELECT * FROM peeps.outgoing_email(1, 1, 'sivers', 'ok', 'ok', ' ', NULL)")
		end
		assert err.message.include? 'body must not be empty'
	end

end

# TESTS NOT USED, (but might need again some day)
__END__
