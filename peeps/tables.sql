...
CREATE TABLE peeps.emailers (
	...
	person_id integer UNIQUE REFERENCES peeps.people(id) ON DELETE RESTRICT,
	was integer UNIQUE REFERENCES peeps.people(id) -- to de-activate, move person_id here
);
...
-- other people's tweets (mentioning me)
...
-- history of people changing my database (via data.sivers.org etc)
CREATE TABLE peeps.changelog (
	id serial primary key,
...
CREATE INDEX changelog_person_id ON peeps.changelog(person_id);
CREATE INDEX changelog_approved ON peeps.changelog(approved);
