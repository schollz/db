...
class TestPeepsAPI < Minitest::Test
	...
	def test_reply_to_email
		...
		assert_equal @j[:message_id][0,8], Time.now.strftime('%Y%m%d')
	...
	def test_create_person_fail
		...
		assert @j[:message].include? 'missing_email'
	end
	...
	def test_person_in_table
		## SEE OTHER API TESTS to test this, becase peeps needs to test on its own
	end
	...
	def test_people_unemailed
		...
		nu = {profile: 'sivers', category: 'sivers', message_id: 'abcdefghijk@yep',
				their_email: 'oompa@loompa.mm', their_name: 'Oompa Loompa', subject: 'yip',
	...
	def test_dead_email_string
		qry("dead_email('derek@sivers.org')")
		assert_equal({ok: 1}, @j)
		qry("get_person(1)")
		assert_equal "DEAD EMAIL: derek@sivers.org\nThis is me.", @j[:notes]
		qry("dead_email('not@here.ie')")
		assert_equal({}, @j)
		qry("dead_email('charlie@bucket.org')")
		assert_equal({ok: 4}, @j)
		qry("get_person(4)")
		assert_equal "DEAD EMAIL: charlie@bucket.org\n", @j[:notes]
	end
	...
	def test_tweets_unknown_top
		qry("peeps.tweets_unknown_top(3)")
		assert @j.include?({name:'Mind the #Quote', handle:'salt'})
		assert @j.include?({name:'J Buckets', handle:'JBuckets3'})
		assert @j.include?({name:'Cath H', handle:'Cat'})
	end
end
