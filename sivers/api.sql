...
BEGIN
	...
	SELECT id INTO new_id FROM sivers.comments
		WHERE uri = new_uri
		AND person_id = new_person_id
		AND html = new_html;
...
CREATE OR REPLACE FUNCTION sivers.reply_to_comment(integer, text,
	OUT status smallint, OUT js json) AS $$
BEGIN
	UPDATE sivers.comments
	SET html = CONCAT(html, '<br><span class="response">',
		replace($2, ':-)', '☺'), ' -- Derek</span>')
	WHERE id = $1;
	status := 200;
	js := row_to_json(r.*) FROM sivers.comments r WHERE id = $1;
	IF js IS NULL THEN m4_NOTFOUND END IF;
